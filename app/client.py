...
def get_value(args: argparse.Namespace) -> None:
    res = requests.get(f'http://{args.node_ip}:{args.node_port}/abci_query?data="{args.key}"')
...
def set_value(args: argparse.Namespace) -> None:
    res = requests.get(f'http://{args.node_ip}:{args.node_port}/broadcast_tx_commit?tx="{args.key}={args.value}={datetime.now()}"', timeout=5)
    # if res.status_code != 200:
    print(res.status_code)
    print(res.text)
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--node_port", default=26657, type=int, required=False)
